...
$PrintTotal = '';
$PrintNbFam = '';
$ControlFamilles = mysql_query('SELECT famille.Id_famille, famille.Nom_famille, sous_famille.Id_sous_famille, sous_famille.Nom_sous_famille FROM famille LEFT JOIN sous_famille ON famille.Id_famille=sous_famille.IdRef_famille WHERE famille.Id_famille="4" ORDER BY Id_famille ASC, Id_sous_famille ASC');
		...
		if (mysql_num_rows($ControlFabInfos) != 0) {
			while ($CheckFabInfos = mysql_fetch_object($ControlFabInfos)) {
				...
				if ($CheckModeleInfos[0] == '0') {
					...
					$CheckCount[$CheckFabInfos->Id_fabricant] = $CheckCount[$CheckFabInfos->Id_fabricant] + $CheckModeleInfos[0];
				}
				...
				if ($CheckCount[$CheckFabInfos->Id_fabricant] > '0') {
				...
				<tr height="450">
					<td align="left" valign="top" colspan="3" bgcolor="#000000">
						<table border="0" cellpadding="0" cellspacing="10" width="690">
							<tr>
								<td align="center" valign="top" style="font-size: 13px; line-height: 15px;">
									...
									<table border="0" cellpadding="5" cellspacing="2">
										...
										if (mysql_num_rows($ControlPrintInfos) != 0) {
											while ($CheckPrintInfos = mysql_fetch_object($ControlPrintInfos)) {
												if ($CheckCount[$CheckPrintInfos->Id_fabricant] > '0') {
