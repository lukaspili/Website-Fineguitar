	 ...
	 <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
	  xmlns:image="http://www.google.com/schemas/sitemap-image/1.1">';
		...
		if (LOCATION){
			...
			$result =	$ocdb->query($query);
			while ($row=mysql_fetch_assoc($result)){				
				$sitemap.=makeUrlTag (SITE_URL.catURL('','',$row['friendlyName']),$insertDate, "weekly", "0.6");
			}
		}
	    ...
	    if (FRIENDLY_URL){
	        $sitemap.=makeUrlTag (SITE_URL."/".u(T_("Advanced Search")).".htm","", "monthly", "0.3");
		    $sitemap.=makeUrlTag (SITE_URL."/".u(T_("Sitemap")).".htm","", "monthly", "0.3");
	    }
	    else{
