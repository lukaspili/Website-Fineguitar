...
class fileCache {
	...
	public function deleteCache($older_than=NULL){
	    $this->addLog('delete cache older than '.$older_than.' for folder: '.$this->cache_path);
	...
	//////////Cache for files individually///////////////////
		...
		private function put($key, $data){
			if ( $this->get($key)!= $data ){//only write if it's different
                ...
                if (!is_dir($cache_dir)){//creating the directory
		            umask(0000);
    ...
    public function removeResource($_target=NULL,$older_than=0) {//remove the done file
        //file?
        if( is_file($_target) ) {
        ...
        if( is_dir($_target) ) {
            if( is_writeable($_target) ) {
                foreach( new DirectoryIterator($_target) as $_res ) {
                    ...
                    if( $_res->isFile() ) {
                        $this->removeResource( $_res->getPathName() );
                    } elseif( $_res->isDir() ) {
                        $this->removeResource( $_res->getRealPath() );
                    }
                    unset($_res);
                }
