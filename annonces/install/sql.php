...
//echo "test prefix ".$TABLE_PREFIX;
	...
	$sqlImport="ALTER TABLE `".$TABLE_PREFIX."postshits` ADD CONSTRAINT `FK_PostsHits_idPost` FOREIGN KEY (`idPost`) REFERENCES `".$TABLE_PREFIX."posts` (`idPost`) ON DELETE CASCADE ON UPDATE CASCADE;";
	mysql_query($sqlImport);
	...
	if ($_POST["SAMPLE_DB"]==1){
        ...
        $sample_import="INSERT INTO `".$TABLE_PREFIX."posts` VALUES  (1,1,1,36,0,'Welcome to Open Classifieds!','<b>Welcome to Open Classifieds!</b><div><b><br></b></div><div>Seems that your new classifieds site is working perfect, that`s great! ;)</div><div><br></div><div>We love to see people happy.</div><div><br></div><div>If you need any other help please go to the forum where we will help you. http://open-classifieds.com/forum/</div><div><br></div><div><br></div><div>PS: To delete this welcome message log in in /admin/ and return to the site, then you will see the tools to delete, edit or spam.</div>','admin@site.com',0,'Barcelona','Chema',0,'127.0.0.1','2010-01-01 18:20:00','00000000','',0);";
        mysql_query($sample_import);
        $sample_import="INSERT INTO `".$TABLE_PREFIX."locations` (friendlyName,name) VALUES 
						('T1', 'test location 1'),
						('T2', 'test location 2'),
						('T3', 'test location 3'),;";
        mysql_query($sample_import);
